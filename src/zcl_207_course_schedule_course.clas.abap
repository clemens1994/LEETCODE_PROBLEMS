CLASS zcl_207_course_schedule_course DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS are_all_req_courses_done
      IMPORTING
        !required_courses     TYPE REF TO cl_object_map
      RETURNING
        VALUE(r_are_all_done) TYPE xfeld .
    METHODS get_course
      RETURNING
        VALUE(course) TYPE int1 .
    METHODS get_required_courses
      IMPORTING
        !courses                TYPE REF TO cl_object_map
      RETURNING
        VALUE(required_courses) TYPE REF TO cl_object_map .
    METHODS get_requirement_course
      RETURNING
        VALUE(requirement_course) TYPE int1 .
    METHODS take_course .
    METHODS check_is_course_done
      RETURNING
        VALUE(is_course_done) TYPE xfeld .
    METHODS constructor
      IMPORTING
        !requirement_course TYPE int1
        !course             TYPE int1 .
    METHODS get_obj_key
      RETURNING
